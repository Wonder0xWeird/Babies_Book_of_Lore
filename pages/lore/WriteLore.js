...
export default function MakeAContribution(props) {
  ...
  async function contribute() {
    if (actorAvatar.name === "A Chaos Portal Opens...") {
      alert(
        "The ANDTHENEUM does not allow Chaos Portals to post or comment. Please step through the portal with an Avatar to make a contribution..."
      )
      return
    } else if (actorAvatar.universe !== "Forgotten Babies Wizard Orphanage") {
      alert(
        "The ANDTHENEUM only allows babies from the Wizard Orphanage to write lore here. Please step through the portal with a baby to make a contribution"
      )
      return
    } else if (input.title === "" || input.body === "" || input.img === "") {
      ...
      if (confirm("The ANDTHENEUM will accept your offering...")) {
        await axios
          .post("/api/lore/post-lore", newContribution)
          .then((result) => {
  ...
  return (
    <Layout
      pageTitle={CCContributionTitle}
      pageDescription={CCContributionDescription}
      pageImg={CCContributionImg}
      pageRunes={CCContributionRunes}
      pageReturn={CCContributionReturn}
    >
      <div className={styles.contributionHolder}>
        <h1>Crafting Lore...</h1>
        <h2>To post your lore, fill out the fields below and click submit.</h2>
        <h3>
          If you have a different image you would like to use, you must first
          upload it to a 3rd party hosting site such as imgur
          (https://imgur.com/) or pinata (https://www.pinata.cloud/) and then
          replace your baby's image URL with the new image's URL in the first
          field below.
        </h3>
        <div className={styles.imgPreviewHolder}>
          <h2>Image Preview:</h2>
          {input.img !== "" ? (
            <img className={styles.imgPreview} src={input.img} />
          ) : (
            <h5>Pending image URL...</h5>
          )}
        </div>
        {isPosting && <Posting />}
        ...
        >
          ...
          <button
            type="submit"
            className="hover-btn float-right"
            onMouseUp={contribute}
            disabled={isposting}
          >
            Contribute
          </button>
